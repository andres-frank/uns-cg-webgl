...
function onLoad() {
	...
	shader_program = Helper.generateProgram(gl, PhongPhong_SpotLight_vertexShaderSource, PhongPhong_SpotLight_fragmentShaderSource);
	...
	// update the uniforms with all these default values
	updateCoefficients();

	// --- Light
	light = { 
		position: {
			gl_location: gl.getUniformLocation(shader_program, 'light.position'),
			radius: 3,
			theta: 40,
			phi: 65
		}, 
		...
		direction: {
			gl_location: gl.getUniformLocation(shader_program, 'light.direction'),
			x: 1,
			y: -1,
			z: 1
		}, 
		...
		color: {
			gl_location: gl.getUniformLocation(shader_program, 'light.color'),
			r: 1,
			g: 0,
			b: 0
		}, 
		...
		cutOffAngle: {
			gl_location: gl.getUniformLocation(shader_program, 'light.cutOffAngle'),
			value: 15
		}, 

		angularAttenuation: {
			gl_location: gl.getUniformLocation(shader_program, 'light.angularAttenuation'),
			value: 0.5
		} 
	};

	// Update the HTML elements
	document.getElementById("lblPhi_light").innerText = light.position.phi;
	document.getElementById("lblTheta_light").innerText = light.position.theta;
	document.getElementById("lblRadius_light").innerText = light.position.radius;
	document.getElementById("lblDirection_light").innerText = `[ ${light.direction.x},${light.direction.y},${light.direction.z} ]`;
	document.getElementById("color_light").value = Utils.rgbFloatToHex(light.color.r, light.color.g, light.color.b);
	document.getElementById("lblCutoff_light").innerText = light.cutOffAngle.value;
	document.getElementById("cutoff_range").value = light.cutOffAngle.value;
	document.getElementById("lblAttenuation_light").innerText = light.angularAttenuation.value;
	document.getElementById("attenuation_range").value = light.angularAttenuation.value;

	// update the uniforms with all these default values
	gl.useProgram(shader_program);
	gl.uniform3fv(light.position.gl_location, Utils.SphericalToCartesian(light.position.radius,light.position.theta,light.position.phi));
	gl.uniform3fv(light.color.gl_location, vec3.fromValues(light.color.r, light.color.g, light.color.b));
	gl.uniform1f(light.cutOffAngle.gl_location, glMatrix.toRadian(light.cutOffAngle.value));
	gl.uniform1f(light.angularAttenuation.gl_location, light.angularAttenuation.value);
	gl.useProgram(null);

	// updateLightUniforms();
	//updateLightPosition();
	//updateLightColor();
	//updateLightCutoffAngle();
...
function updateLightPosition() {
	...
	lightPos_value = Utils.SphericalToCartesian(
		...
		light.position.theta,
		light.position.phi
	);

	gl.useProgram(shader_program);
	gl.uniform3fv(light.position.gl_location, lightPos_value);
	gl.useProgram(null);
}

function onLightInteraction(event) {
	gl.useProgram(shader_program);

	switch (event.id) {

		// case "light_position":
		// 	newvalue = Utils.SphericalToCartesian(
		// 		light.position.radius,
		// 		light.position.theta,
		// 		light.position.phi
		// 	);

		// 	gl.uniform3fv(light.position.gl_location, newvalue);
		// 	break;

		case "attenuation_range":
			light.angularAttenuation.value = event.value;
			document.getElementById("lblAttenuation_light").innerText = event.value;

			gl.uniform1f(light.angularAttenuation.gl_location, light.angularAttenuation.value);
			break;

		case "cutoff_range":
			light.cutOffAngle.value = glMatrix.toRadian(event.value);
			gl.uniform1f(light.cutOffAngle.gl_location, light.cutOffAngle.value);

			document.getElementById("lblCutoff_light").innerText = event.value;
			break;

		case "color_light":
			let rgbf = Utils.hexToRgbFloat(event.value);

			light.color.r = rgbf.r;
			light.color.g = rgbf.g;
			light.color.b = rgbf.g;

			newvalue = vec3.fromValues(light.color.r, light.color.g, light.color.b);
			gl.uniform3fv(light.color.gl_location, newvalue);

			document.getElementById("lblColor_light").innerText = document.getElementById("color_light").value;
			break;
	}
...
function lightEventHandler(event) {
	...
	switch(event.key) {
		case "ArrowUp":
			newvalue = light.position.phi + 5;
			if ( newvalue < 180 ) light.position.phi = newvalue;
			document.getElementById("lblPhi_light").innerText = light.position.phi;
			break;

		case "ArrowDown":
			newvalue = light.position.phi - 5;
			if ( newvalue > 0 ) light.position.phi = newvalue;
			document.getElementById("lblPhi_light").innerText = light.position.phi;
			break;

		case "ArrowRight":
			light.position.theta = (light.position.theta + 5) % 360;
			document.getElementById("lblTheta_light").innerText = light.position.theta;
			break;
